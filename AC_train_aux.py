...
g = Game.Game(FPS=120, stepTime=0.15, auxiliaryReward=True)
...
while True:
    ...
    if g.LOCK:
        ...
        g.reset()
        if count % 25 == 0:
            print('average score in last 25 rounds: {} \n network parameter saved.'.format(np.mean(scores[-25:])))
        if count % 100 == 0:
            torch.save(agent.Actor.state_dict(), './results/AC_parameters_aux.pkl')
        count += 1
    if count > n_trails:
        break
